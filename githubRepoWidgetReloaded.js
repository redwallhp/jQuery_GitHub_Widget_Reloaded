jQuery(document).ready(function($){
	...
	$('head').append(
		...
		+'.github-user-repos-widget div.github-box { margin-bottom: 10px; }'
		+'</style>'
	);


	function widget(repo) {
		...
		if (repo.homepage != null) {
			var link = repo.homepage;
		} else {
			var link = repo.html_url;
		}

		return '<div class="github-box repo">'
		+'<div class="github-box-title">'
		+'<h3>'
		+'<a class="owner" href="' + repo.owner.html_url + '">' + repo.owner.login + '</a>'
		+'/'
		+'<a class="repo" href="' + repo.html_url + '">' + repo.name + '</a>'
		+'</h3>'
		+'<div class="github-stats">'
		+'<a class="watchers" href="' + repo.html_url + '/watchers">' + repo.watchers + '</a>'
		+'<a class="forks" href="' + repo.html_url + '/network/members">' + repo.forks + '</a>'
		+'</div>'
		+'</div>'
		+'<div class="github-box-content">'
		+'<p class="description"><span>'+repo.description+'</span> &mdash; <a href="' + repo.html_url + '#readme">Read More</a></p>'
		+'<p class="link"><a href="'+link+'">'+link+'</a></p>'
		+'</div>'
		+'<div class="github-box-download">'
		+'<p class="updated">'+updated+'</p>'
		+'<a class="download" href="' + repo.html_url + '/zipball/master">Download as zip</a>'
		+'</div>'
		+'</div>';
	...
	$('.github-user-repos-widget').each(function() {
		var container = $(this);
		var user = container.data('user');
		$.getJSON('https://api.github.com/users/'+user+'/repos?type=owner&callback=?', function(res) {
			for (var i = 0; i < res.data.length; i++) {
				var repo = res.data[i];
				container.append(widget(repo));
			}
		});
	});


	$('.github-repo-widget').each(function() {
		var container = $(this);
		var repo = container.data('repo');
		$.getJSON('https://api.github.com/repos/'+repo+'?callback=?', function(res) {
			var repo = res.data;
			container.append(widget(repo));
		});
	});
